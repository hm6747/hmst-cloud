...
/*  private static final String SERVER_PROPERTIES_LINUX = "/opt/settings/server.properties";
  private static final String SERVER_PROPERTIES_WINDOWS = "C:/opt/settings/server.properties";*/
    ...
    public void initialize() {
        try {
            InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream(SERVER_PROPERTIES_LINUX);
            if (in == null) {
                in = DefaultServerProvider.class.getResourceAsStream(SERVER_PROPERTIES_LINUX);
            }
            if (in == null) {
                logger.warn("{} not found from classpath!", SERVER_PROPERTIES_LINUX);
            }
            initialize(in);
        } catch (Throwable ex) {
